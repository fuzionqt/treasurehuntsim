...
AutoFarm.MouseButton1Click:Connect(
	function()
		if Click1 then
			Click1 = false
			Farm = false
...
AutoRebirth.MouseButton1Click:Connect(
	function()
		if Click1 then
			Click1 = false
			Rebirth = false
	...
	while true do
		...
		if Rebirth == true then
			...
			if tonumber(a) > tonumber(b) then 
				...
				repeat
					wait(.1)
				until game.Players.LocalPlayer.PlayerGui.Gui.Popups.GiveReward.Visible == true
					...
					if Next == true then
						local Success, Problem =
							pcall(
								function()
								if
									game.Players[game.Players.LocalPlayer.Name].PlayerGui.Gui.Popups.BackpackFull.Visible ==
									true
								then
									Sell()
								end
								...
								repeat
									...
									Character:WaitForChild(ToolBox.Text)["RemoteClick"]:FireServer(
									game.Workspace.SandBlocks[SandName]
									)
									wait()
